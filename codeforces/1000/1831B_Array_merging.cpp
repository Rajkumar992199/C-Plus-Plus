...
int main() {
    ...
    while (t--) {
        ...
        cin >> n;
        vector<long long> a(n), b(n), longest_subarray_a(2 * n + 1, 0), longest_subarray_b(2 * n + 1, 0);
        ...
        for (int i = 1; i <= 2 * n; i++) max_freq = max(max_freq, longest_subarray_a[i] + longest_subarray_b[i]);
        cout << max_freq << endl;
    }
    return 0;
}
